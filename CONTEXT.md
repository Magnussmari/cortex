...
> The part of the stack we most often get tripped up on. The rule: **two signing systems, both sovereign per-principal, never cross-issued** — nobody mints credentials for anybody else. Decision: `docs/adr/0013-sovereign-federation-model.md` (accepted), supersession of Model A in `docs/adr/0015-two-tier-onboarding-and-admission-gate.md`.

![How a stack joins a network under sovereign Model-B federation](docs/diagrams/cortex-network-join.png)

**Sovereign federation (Model B)**:
The one and only federation model (ADR-0013): a **principal** federates by rooting their NATS identity in **their own NSC operator** — there are **no hub-minted guest accounts**, and no operator trusts another operator's JWTs. Sovereignty is *structural*: it falls out of (a) a secret-authenticated leaf pipe and (b) each side running its own export/import — not from a trust protocol.
_Avoid_: Model A, hub-minted creds, guest account, "the hub issues you a `.creds`" (retired — ADR-0015).

**The two signing systems** (the trip-up — both sovereign, both per-principal, never cross-issued):
1. **Application identity (the wire).** Each **stack** mints its own Ed25519 signing key (`cortex provision-stack generate`) and signs every envelope via `runtime.publish` (see **Stack signing identity**). The **registry** is only the public *directory* of these pubkeys — it signs nothing.
2. **NATS transport (the connection).** Each **principal**'s own **NSC operator** signs their own account, which signs their own leaf-user creds (M1 connection auth). No cross-operator signing, ever.

**Where signing does NOT happen.** The **registry** (`network.meta-factory.ai`) is the *DNS of the federation* — it stores + serves pubkeys and the network descriptor and **mints/signs nothing**. The network/hub admin **issues no credential to a joiner**. The only thing handed across the boundary is a **leaf shared secret** — a transport PSK (like a Wi-Fi password) that lets a leaf attach to the hub's medium — never an identity credential.
_Avoid_: "the registry signs/issues creds"; "the hub admin mints my account"; treating the leaf secret as an identity.
...
2. **Register** — `cortex provision-stack register` posts your pubkey + capabilities, signed by that key (proof-of-possession). The registry stores the directory entry; it does not vouch for you cryptographically beyond holding it.
...
4. **Get admitted** — the admin approves your **roster membership** and hands you the **leaf shared secret + which side hosts the hub**. The one two-party handoff; it mints nothing.
5. **Join** — `cortex network join <network> --apply`: ensures your own NSC operator, renders the secret-auth leaf bound to *your own local account*, wires your half of export/import, restarts NATS + the runtime.
6. **Verify** — `cortex network status` → leaf link `established`, peers resolved from the roster.

**TCP/IP mapping**: your NSC operator = you own your keys (your device's identity) · the registry = DNS (pin one resolver; resolve names→pubkeys) · the leaf shared secret = the PSK to attach to the medium · `cortex network join <network>` = the DHCP-style plug-in · **scope** `local`/`federated`/`public` = localhost/LAN/internet — same interface, different reach.

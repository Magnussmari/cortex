...
| Security posture | `security.signing: off \| permissive \| enforce` (TC-0, #628; knob mapping in `src/common/security-posture.ts`). Non-empty `signed_by` chains are verified (structural trust + ed25519, `verifySignedByChain` in `src/bus/verify-signed-by-chain.ts`) where chain verification is wired — the runner dispatch-listener (`src/runner/dispatch-listener.ts`), the bus-dispatch-listener, and the review consumer (`src/bus/review-consumer.ts`); a FORGED stamp is dropped there under every posture. But under `off`/`permissive`, EMPTY chains are accepted (`rejectEmpty: false`): an attacker on the bus simply omits `signed_by` and falls through to the policy gate. **`signed_by` therefore cannot be relied on as a trust gate below `enforce` — the effective trust boundary under `off` is bus-layer auth (NATS leaf-node/user auth).** Do not deploy `signing: off` to a multi-tenant or open-bus environment expecting signing to gate admission (cortex#1000). Default is seed-aware: a stack with `stack.nkey_seed_path` configured and `signing` unset boots `permissive`; only an explicit `signing: off` disables outbound signing on a seed-configured stack. Only `enforce` rejects unsigned envelopes. |

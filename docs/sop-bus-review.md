...
3. Cortex provisions the `bus.review.stream.name` stream, default `CODE_REVIEW`, for `local.{operator}.{stack}.tasks.code-review.>`.
4. Cortex provisions one durable per code-review-capable agent: `cortex-review-consumer-{operator}-{agent}`.
...
- Stream subject includes `local.<operator>.<stack>.tasks.code-review.>`, or a deliberately broader existing subject such as `local.>`.
...
- Stream missing: run `arc nats provision-streams --network <operator> --agent <agent>` or restart Cortex with a working `nats.url`.
- Durable missing: same provisioning command; durable names are operator and agent scoped.
- Subject mismatch: confirm publishers use `local.<operator>.<stack>.tasks.code-review.<flavor>` and cortex logs the same stack id at boot.

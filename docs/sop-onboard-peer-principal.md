...
Your stack's local NATS bus must be **operator-mode** (it must define your NSC operator, the system account when JetStream is enabled — almost always; see the note below, and the resolver preload including the federation account) before it can bind a leaf link to an operator-mode hub. A hard-isolated anonymous bus (no NSC operator, no accounts) cannot federate.

> **What operator-mode means here.** Your bus config must include the NSC operator JWT, `system_account` (see the JetStream note below), `resolver: MEMORY`, and `resolver_preload` containing your federation account (and, once landed, your agents account). Keep your stack's own `server_name`, `listen` port, `jetstream.domain`, and `http` monitor port — `cortex network join` renders its own leaf include for your stack.

> **JetStream requires a system account.** A cortex stack runs **JetStream by default** (the `jetstream{}` block in your bus config). In operator mode JetStream needs a configured **`system_account`** for its internal `$JS`/`$SYS` subscriptions — without one, `nats-server` fatals at boot with `Can't start JetStream: setting up internal jetstream subscriptions failed: system account not setup`. So for any JetStream-enabled stack the SYS account is **required, not optional**. **Known gap ([cortex#1333](https://github.com/the-metafactory/cortex/issues/1333), PR [#1357](https://github.com/the-metafactory/cortex/pull/1357)):** folding SYS provisioning into `cortex network provision` (so onboarding stays cortex-commands-only) is tracked and in review, not yet merged — no schedule is implied. Until it lands, set the SYS account up manually: create it with `nsc add account SYS` and ensure `system_account` + `system_account_jwt` are present in your stack's `nats_infra` block. `cortex network provision` (Step 4a) exports them when the SYS account already exists in your operator at provision time; if you add SYS afterwards, set those two fields yourself. `make-live` / `join` then render the system account into the bus config. Verified 2026-06-29 during a real peer-stack bring-up on a fresh Hetzner Cloud VM (Debian 12, nats-server 2.x): boot first failed with the exact fatal quoted above, then `nsc add account SYS` → both fields into `nats_infra` → `make-live` rendered `system_account` and the same stack booted clean. It is only optional if you remove the `jetstream{}` block.
